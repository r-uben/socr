...
def common_options(f):
    ...
    f = click.option("--profile", type=str, help="Load ~/.config/smart-ocr/{profile}.yaml")(f)
    return f
...
def cli(ctx: click.Context) -> None:
    ...
    Usage:
        ...
        smart-ocr batch ./papers/ [OPTIONS]    # Process directory
...
def batch(pdf_dir: Path, output_dir: Path | None, limit: int | None, **kwargs) -> None:
    ...
    Example:
        smart-ocr batch ~/Papers/ -o ./results/
        smart-ocr batch ~/Papers/ --dry-run
        smart-ocr batch ~/Papers/ --reprocess --quiet
    """
    from smart_ocr.pipeline.processor import StandardPipeline
