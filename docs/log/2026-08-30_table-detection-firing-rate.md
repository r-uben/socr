...
21-page sample the manifest itself lists. For each page, two independent calls:

- `page.find_tables()` — PyMuPDF's ruled-table detector.
- `socr.tables.reconstruct.has_numeric_columns(page)` — socr's second-pass gate,
  reached only when the first returns nothing.
...
"Second-pass only" is the number that matters: pages routed to the table path on the
numeric-lane heuristic alone, with no ruled table found. **78 of 528 pages, 15%.**
...
- GH-326's re-measurement wants a trustworthy denominator, which is what this is.
