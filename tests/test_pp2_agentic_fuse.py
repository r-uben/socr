...
class TestPhase4cGate:
    """dual_pass_tables must not fire in agentic mode (tables handled in-loop)."""

    def test_dual_pass_tables_skipped_in_agentic_mode(self, tmp_path: Path) -> None:
        pdf_path = _real_pdf(tmp_path, page_count=1)
        config = _make_config(
            agentic=True,
        ...
        with (
            patch("socr.pipeline.orchestrator.get_engine", return_value=_gemini_engine_mock()),
            patch.object(pipeline, "_phase_dual_pass_tables", side_effect=_spy),
        ):
            pipeline.process(pdf_path, tmp_path)

        assert not call_count, (
            f"_phase_dual_pass_tables must NOT be called in agentic mode; "
            f"called {len(call_count)} time(s)"
        )

    def test_dual_pass_tables_runs_in_non_agentic_mode(self, tmp_path: Path) -> None:
        """_phase_dual_pass_tables still runs in non-agentic mode."""
        pdf_path = _real_pdf(tmp_path, page_count=1)
        config = _make_config(
            agentic=False,
            dual_pass_tables=True,
        ...
        with (
            patch("socr.pipeline.orchestrator.get_engine", side_effect=_fake_engine),
            patch.object(pipeline, "_phase_dual_pass_tables", side_effect=_spy),
        ):
            pipeline.process(pdf_path, tmp_path)

        assert call_count, (
            "_phase_dual_pass_tables must be called in non-agentic mode when dual_pass_tables=True"
        )
